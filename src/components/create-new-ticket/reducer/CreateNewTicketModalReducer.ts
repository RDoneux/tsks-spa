...
type CreateTicketReducerType = (typeof CreateTicketReducerType)[keyof typeof CreateTicketReducerType];
...
export function createTicketReducer(state: Partial<ITicket>, action: CreateTicketReducerAction): Partial<ITicket> {
    switch (action.type) {
...
export function updateDescription(description: string): CreateTicketReducerAction {
    return { type: CreateTicketReducerType.UPDATE_DESCRIPTION, payload: description }
}
...
export function updateColumn(columnId: IColumn): CreateTicketReducerAction {
    return { type: CreateTicketReducerType.UPDATE_COLUMN, payload: JSON.stringify(columnId) };
}